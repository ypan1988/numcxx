...
TEST_F(ComputeStridesTest, RowMajor3D) {
  ...
  const size_t total_size = ComputeStrides(&strides, shape, MemoryOrder::kRowMajor);
...
TEST_F(ComputeStridesTest, ColumnMajor2D) {
  ...
  const size_t total_size = ComputeStrides(&strides, shape, MemoryOrder::kColMajor);
...
TEST_F(ComputeStridesTest, EmptyShape) {
  const size_t total_size = ComputeStrides(&strides, {}, MemoryOrder::kRowMajor);
